...
def save_json(
    data: dict,
    file_name: str,
    save_dir: str = str(settings.OUTPUT_DIR),
) -> None:
...
def load_json(file_name: str, save_dir: str = str(settings.OUTPUT_DIR)) -> dict:
    """Load a JSON file.
...
def save_model(model: object, model_path: Union[str, Path]) -> None:
...
def check_if_artifact_exists(
    ...
    try:
        if get_artifact(artifact_name, project, entity):
            return True
